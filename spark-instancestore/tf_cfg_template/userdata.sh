...
# 配置 benchmark 工具：
IPADDR=localhost
sed -i.bak "s/localhost:2181\/;serviceDiscoveryMode=zooKeeper;zooKeeperNamespace=hiveserver2?tez.queue.name=default/${IPADDR}:10000\//" $TEST_TOOL_FOLDER/hive-testbench/tpcds-setup.sh
...
cd $TEST_TOOL_FOLDER/hive-testbench/spark-queries-tpcds
LIST=$(ls *.sql)
