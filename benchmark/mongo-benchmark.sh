...
submit_task(){
    ...
    ## Profile
    WORKLOAD_PROFILE=${2}
    RESULT_FILE="${RESULT_PATH}/${SUT_NAME}_${INSTANCE_TYPE}_${OS_TYPE}_${INSTANCE_IP_MASTER}-${WORKLOAD_PROFILE}-load.txt"
    RESULT_FILE1="${RESULT_PATH}/${SUT_NAME}_${INSTANCE_TYPE}_${OS_TYPE}_${INSTANCE_IP_MASTER}-${WORKLOAD_PROFILE}-run.txt"

    for i in {1..3}
    do
        ### 加载数据, load
        /root/ycsb-0.17.0/bin/ycsb load mongodb -P $(dirname $0)/${WORKLOAD_PROFILE} -p mongodb.url=${MONGO_URL} \
            >> ${RESULT_FILE}
          
        ### 执行 Benchmark, run
        /root/ycsb-0.17.0/bin/ycsb  run mongodb -P $(dirname $0)/${WORKLOAD_PROFILE} -p mongodb.url=${MONGO_URL} \
            >> ${RESULT_FILE1}
...
submit_task ${1} workload_mongo_readonly
submit_task ${1} workload_mongo_updateonly
submit_task ${1} workload_mongo_mixed
