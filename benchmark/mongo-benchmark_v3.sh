...
submit_task(){
	...
	### 加载数据, load
	echo "[Info] Load data with $(nproc) threads ..." >> ${RESULT_FILE}
	/root/ycsb-0.17.0/bin/ycsb load mongodb -P $(dirname $0)/workload_mongo_readonly -p mongodb.url=${MONGO_URL} -threads $(nproc) >> ${RESULT_FILE}
    ...
    ### 完成测试后删除数据库
    remove_database && \
    echo "[Info] All benchmark tests completed, remove database." >> ${RESULT_FILE1}
}
