...
cd hdf5-hdf5-1_10_7/
./configure --enable-cxx --prefix=/usr/ && make -j$(nproc)
make install 
find /usr/ -name "*hdf5*"
...
models="BERT_pytorch,hf_Bert,hf_Bert_large,hf_GPT2,hf_Albert,hf_Bart,hf_BigBird,hf_DistilBert,hf_GPT2_large,\
       dlrm,hf_T5,mnasnet1_0,mobilenet_v2,mobilenet_v3_large,squeezenet1_1,timm_efficientnet,shufflenet_v2_x1_0,\
       timm_regnet,resnet50,soft_actor_critic,phlippe_densenet,resnet152,resnet18,resnext50_32x4d,densenet121,\
       phlippe_resnet,doctr_det_predictor,timm_vovnet,alexnet,doctr_reco_predictor,vgg16,dcgan,yolov3,\
       pytorch_stargan,hf_Longformer,timm_nfnet,timm_vision_transformer,timm_vision_transformer_large,\
       nvidia_deeprecommender,demucs,tts_angular,hf_Reformer,pytorch_CycleGAN_and_pix2pix,functorch_dp_cifar10,\
       pytorch_unet"
python3 run_benchmark.py cpu --model $models \
  ...
  1> torch_compile_inductor_$instance_type_$(date +%Y%m%d_%H%M%S).log 2>&1
