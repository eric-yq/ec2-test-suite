...
public class CustomNativeLibraryDemo {
    ...
    /**
     * 加载自定义 native libraries（多架构支持）
     */
    private void loadNativeLibraries() {
        try {
            // 检测当前架构
            String currentArch = detectArchitecture();
            logger.info("Detected architecture: {}", currentArch);
            ...
            // 从 resources 中提取对应架构的 .so 文件到共享临时目录
            extractNativeLibrary("libmath_ops_" + currentArch + ".so", "libmath_ops.so");
            extractNativeLibrary("libstring_ops_" + currentArch + ".so", "libstring_ops.so");
            extractNativeLibrary("libsystem_ops_" + currentArch + ".so", "libsystem_ops.so");
            ...
            logger.info("All custom native libraries loaded successfully for architecture: {}", currentArch);
    ...
    private String detectArchitecture() {
        ...
        logger.debug("System property os.arch: {}", osArch);
        ...
        if (osArch.contains("amd64") || osArch.contains("x86_64")) {
            ...
            logger.warn("Unknown architecture: {}, defaulting to x86_64", osArch);
            return "x86_64";
        }
    }
    
    /**
     * 从 JAR 中提取 native library 到共享临时目录（多架构支持）
     */
    private void extractNativeLibrary(String sourceLibraryName, String targetLibraryName) throws Exception {
        String resourcePath = "/native/" + sourceLibraryName;
        
        try (InputStream is = getClass().getResourceAsStream(resourcePath)) {
            if (is == null) {
                // 如果找不到架构特定的库，尝试查找通用库
                String fallbackPath = "/native/" + targetLibraryName;
                logger.warn("Architecture-specific library not found: {}, trying fallback: {}", resourcePath, fallbackPath);
                
                try (InputStream fallbackIs = getClass().getResourceAsStream(fallbackPath)) {
                    if (fallbackIs == null) {
                        throw new RuntimeException("Native library not found in resources: " + resourcePath + " or " + fallbackPath);
                    }
                    extractLibraryFromStream(fallbackIs, targetLibraryName);
                }
            } else {
                extractLibraryFromStream(is, targetLibraryName);
            }
            
            logger.info("Extracted native library: {} -> {}", sourceLibraryName, targetLibraryName);
        }
    }
     ...
     * 从输入流提取库文件
     */
    private void extractLibraryFromStream(InputStream is, String targetLibraryName) throws Exception {
        // 创建临时文件在共享目录中
        File tempFile = new File(sharedTempDir, targetLibraryName);
        ...
        logger.debug("Library extracted to: {}", tempFile.getAbsolutePath());
    }
