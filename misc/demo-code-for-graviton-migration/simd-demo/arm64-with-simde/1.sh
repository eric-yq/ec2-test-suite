...
s3fs e2b-arm02-fc-env-pipeline-022633198646 $envd_dir -o iam_role=auto,allow_other,ro,umask=0022
s3fs e2b-arm02-fc-kernels-022633198646 $kernels_dir -o iam_role=auto,allow_other,ro,umask=0022,use_cache=/tmp/s3fs_cache_kernels
s3fs e2b-arm02-fc-versions-022633198646 $fc_versions_dir -o iam_role=auto,allow_other,ro,umask=0022,use_cache=/tmp/s3fs_cache_versions

# These variables are passed in via Terraform template interpolation
aws s3 cp "s3://e2b-arm02-cluster-setup-022633198646/run-consul-41300.sh" /opt/consul/bin/run-consul.sh
aws s3 cp "s3://e2b-arm02-cluster-setup-022633198646/run-nomad-445f5.sh" /opt/nomad/bin/run-nomad.sh
...
{
    "auths": {
        "022633198646.dkr.ecr.us-east-1.amazonaws.com": {
...
/opt/consul/bin/run-consul.sh --client \
    --consul-token "ede475a7-6526-0581-32a9-11520fe80584" \
    --cluster-tag-name "client-cluster" \
    --enable-gossip-encryption \
    --gossip-encryption-key "hshJsPSUsVmYL9nrWqRH/v4IEJXabV9purxOLgFJTjs=" \
    --dns-request-token "7c7b2c92-e46c-0287-299a-2b1e65405cc3" &

/opt/nomad/bin/run-nomad.sh --client --consul-token "ede475a7-6526-0581-32a9-11520fe80584" &
