...
##########################################################################################
# 安装 BenchmarkSQL 工具
apt install -y openjdk-17-jdk unzip ant
...
MEM_FREE_GB=$(obclient -h127.0.0.1 -P2881 -uroot -p${ROOT_PASSWORD} -Doceanbase -A -N -s -e \
  "SELECT 
    round((mem_capacity - mem_assigned)/1024/1024/1024, 2) AS mem_free_gb 
...
obclient -h127.0.0.1 -P2881 -uroot -p${ROOT_PASSWORD} -Doceanbase -A -N -s -e \
  "CREATE RESOURCE UNIT test MAX_CPU=${CPU_FREE_NUM}, MIN_CPU=${CPU_FREE_NUM}, MEMORY_SIZE=${MEM_FREE_GB}G;"
obclient -h127.0.0.1 -P2881 -uroot -p${ROOT_PASSWORD} -Doceanbase -A -N -s -e \
