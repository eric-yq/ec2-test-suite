...
## -- 查询各个表占用的磁盘容量
mysql --comments  -h ${tidb_host} -P ${tidb_port} -u root -e "SELECT table_name, CONCAT(ROUND(data_length/1024/1024, 2), ' MB') as data_size FROM information_schema.tables WHERE table_schema = 'tpch300' ORDER BY data_length DESC;"

## -- 查询各个表的记录数
mysql --comments  -h ${tidb_host} -P ${tidb_port} -u root -e "SELECT table_name, table_rows FROM information_schema.tables  WHERE table_schema = 'tpch300' ORDER BY table_name;"

mysql --comments  -h ${tidb_host} -P ${tidb_port} -u root -e "SELECT * FROM information_schema.tiflash_replica WHERE TABLE_SCHEMA = 'tpch300' ORDER BY TABLE_NAME;"
