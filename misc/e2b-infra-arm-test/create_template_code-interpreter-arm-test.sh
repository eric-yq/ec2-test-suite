...
echo "Creating ECR repository e2bdev/code-interpreter-arm-test/$TEMPLATE_ID..."
aws ecr create-repository --repository-name e2bdev/code-interpreter-arm-test/$TEMPLATE_ID --region $AWSREGION || true
if [ $? -ne 0 ]; then
    echo "Note: Repository may already exist or there was an error"
fi

# Pull the base Docker image
echo "Pulling e2bdev/code-interpreter-arm-test Docker image..."
docker pull 022633198646.dkr.ecr.us-east-1.amazonaws.com/e2bdev/code-interpreter-arm-test:latest
if [ $? -ne 0 ]; then
    echo "Error: Failed to pull e2bdev/code-interpreter-arm-test Docker image"
    exit 1
fi
...
ECR_REPOSITORY="$AWS_ACCOUNT_ID.dkr.ecr.$AWSREGION.amazonaws.com/e2bdev/code-interpreter-arm-test/$TEMPLATE_ID:$BUILD_ID"
echo "Tagging Docker image as $ECR_REPOSITORY..."
docker tag e2bdev/code-interpreter-arm-test:latest $ECR_REPOSITORY
if [ $? -ne 0 ]; then
