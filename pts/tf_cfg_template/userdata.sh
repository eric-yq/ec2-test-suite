...
tar czfP ${DATA_DIR}-standard.tar.gz ${DATA_DIR}


## 执行基准测试(更多)
echo "[INFO] Step2: Start to perform more PTS tests related to complex workload..."
tests="blogbench nginx memtier-benchmark cassandra tjbench vvenc ncnn libxsmm spark"
for testname in ${tests} 
do
    phoronix-test-suite batch-benchmark ${testname} > ${PTS_RESULT_DIR}/${testname}.txt
    echo "${testname}.txt:" >> ${DATA_DIR}/pts-result-url-summary.txt
...
## 执行基准测试(更多)--20240110
echo "[INFO] Step3: Start to perform more new PTS tests related to complex workload..."
tests="influxdb rocksdb clickhouse tensorflow pytorch"
for testname in ${tests} 
do
    phoronix-test-suite batch-benchmark ${testname} > ${PTS_RESULT_DIR}/${testname}.txt
    echo "${testname}.txt:" >> ${DATA_DIR}/pts-result-url-summary.txt
...
echo "[INFO] Step3: Complete ALL PTS TESTS."
